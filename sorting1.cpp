...
int main() {
	...
	*/
	...
	for (i = 1; i <= 5; i++) {
		...
		case 2: {	QueryPerformanceCounter(&li);		//计算算法效率时间
			long long f_start = li.QuadPart;
		...
		case 3: {	QueryPerformanceCounter(&li);		//计算算法效率时间
			long long f_start = li.QuadPart;
		...
		case 5: {	QueryPerformanceCounter(&li);		//计算算法效率时间
			...
			long long f_end = li.QuadPart;
			f_time = (f_end - f_start);
			timeout << "SHELL排序 time=" << f_time << endl;
